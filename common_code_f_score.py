...
def Naive_Bayes(data_train, data_test, target_train, target_test):
    ...
    return f1_score(target_test, pred,average='micro')
...
def AdaBoost(data_train, data_test, target_train, target_test):
    ...
    return metrics.f1_score(target_test, pred,average='micro')
...
os.getcwd()
os.chdir('G:\\Elastic Search')
data=pd.read_csv(r'csv_result-ar6.csv')
n=len(data)
m=len(data.columns)
for i in range(m):
